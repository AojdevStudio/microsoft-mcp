...
def is_dangerous_rm_command(command):
    ...
    if re.search(
        r"\brm\s+.*-[a-z]*r\b", normalized
    ):  # If rm has recursive flag (only actual flags)
    ...
    try:
        ...
        if tool_name == "Bash":
            ...
            if is_dangerous_rm_command(command):
                ...
                print(
                    "  • For directories: rm -r dirname (no -f flag)", file=sys.stderr
                )
                print("  • Use specific paths instead of wildcards", file=sys.stderr)
                print(
                    "  • Consider using trash/archive instead of permanent deletion",
                    file=sys.stderr,
                )
                sys.exit(2)  # Exit code 2 blocks tool call and shows error to Claude
        ...
        if is_command_file_access(tool_name, tool_input):
            ...
            if template_file and os.path.exists(template_file):
                ...
                print(
                    "   • 6-part structure: YAML frontmatter, heading, description, arguments, instructions, context",
                    file=sys.stderr,
                )
                print(
                    "   • Use action verbs and keep descriptions under 80 characters",
                    file=sys.stderr,
                )
                print(
                    "   • Include dynamic data gathering with ! commands",
                    file=sys.stderr,
                )
                print("   • Reference files with @ syntax", file=sys.stderr)
                print(
                    "   • Follow consistent naming and formatting patterns",
                    file=sys.stderr,
                )
                print("", file=sys.stderr)
        ...
        if check_date_awareness(tool_name, tool_input):
            if not check_recent_date_command():
                ...
                print("", file=sys.stderr)
                print(
                    "🗓️  Common date hallucination patterns detected:", file=sys.stderr
                )
                print(
                    "   • Month/Year references (e.g., 'January 2025')", file=sys.stderr
                )
                print("   • Quarter references (e.g., 'Q1 2025')", file=sys.stderr)
                print(
                    "   • Relative dates (e.g., 'next quarter', 'by end of 2025')",
                    file=sys.stderr,
                )
                print("", file=sys.stderr)
                print(
                    "✅ To proceed accurately: Run the Bash tool with command 'date' first",
                    file=sys.stderr,
                )
